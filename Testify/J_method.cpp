...
void chatterCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    if(odom_parameter.trans<0.01)
    {
      odom_parameter.rot1 = 0;
    }
    else
    {
      odom_parameter.rot1 = angle_diff(atan2(now_motion.y-last_motion.y, now_motion.x-last_motion.x), last_motion.theta);
    }
    ...
    //set the error
    if(2*sqrt(COV(0, 0)) < 20 && 2*sqrt(COV(1, 1)) < 20 && 2*sqrt(COV(2, 2)) < 10)
    {
      real_motion.x = MU(0);
      real_motion.y = MU(1);
